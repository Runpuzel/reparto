...
| 5 | **Error prevention** | Form **validation** before submission (email, password rules, Student ID, phone, MoMo), confirm dialogs before destructive actions (delete product), stock caps the quantity stepper, server-side checks (RLS, stock, approval). | `utils/validators.dart`, `product_form_screen.dart`, `product_detail_screen.dart` |
